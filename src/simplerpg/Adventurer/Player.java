...
public class Player extends BaseObj
{
	...
	public Player()
	{
		...
		attack3.setImage(new Image(ImgLink+"shooting"+"_"+"player"+"_"+NTS(Launcher.PlayerId)+".png"));
		attackani3 = new SpriteAnimation(attack3, Duration.millis(200), 11, 11, State.getBGWIDTH(), State.getBGHEIGHT(),State.getBGWIDTH(),State.getBGHEIGHT());
	...
	public void Attack03(int direction,double X,double Y)
	{
		curDirection = direction;
		
		if(curHP<0 || hurtedani.getStatus()!=Status.STOPPED || attackani2.getStatus()!=Status.STOPPED  || attackani1.getStatus() !=Status.STOPPED || attackani3.getStatus() == Status.RUNNING) return;
		
		
				
		SetUpAnimation(
				attack3,
				new Image(ImgLink+"shooting"+"_"+"player"+"_"+NTS(Launcher.PlayerId)+".png"),
				attackani3,
				Duration.millis(500*State.getAttackSpeed()),
				6,
				6,
		...
		attackani3.setOnFinished(e ->{if(cf.root.getChildren().indexOf(attack3)>=0) cf.root.getChildren().remove(attack3);});
		attackani3.play();
	}
	public void Death()
	{
